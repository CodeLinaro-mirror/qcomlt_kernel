...
// TODO: check all SSPP TOP0 and other INTR because SSPP changed
...
static const struct dpu_ctl_cfg sm8750_ctl[] = {
	{
		.name = "ctl_0", .id = CTL_0,
		.base = 0x15000, .len = 0x1000, // base are ok, but several
						// other changes
...
static const struct dpu_sspp_cfg sm8750_sspp[] = {
	{
		...
		.base = 0x4000, .len = 0x344, // base and len ok, rest maybe ok
...
static const struct dpu_pingpong_cfg sm8750_pp[] = {
	{
		...
		.name = "pingpong_10", .id = PINGPONG_10,
		.base = 0x7e000, .len = 0,
		.features = BIT(DPU_PINGPONG_DITHER),
		.sblk = &sc7280_pp_sblk,
		.merge_3d = MERGE_3D_5,
	}, {
		.name = "pingpong_11", .id = PINGPONG_11,
		.base = 0x7e400, .len = 0,
		.features = BIT(DPU_PINGPONG_DITHER),
		.sblk = &sc7280_pp_sblk,
		.merge_3d = MERGE_3D_5,
	}, // look ok, some new registers added
};

static const struct dpu_merge_3d_cfg sm8750_merge_3d[] = {
	{
		.name = "merge_3d_0", .id = MERGE_3D_0,
		.base = 0x4e000, .len = 0x1c, // 2x new register: clk control
	}, {
...
static const struct dpu_wb_cfg sm8750_wb[] = {
	{
		.name = "wb_2", .id = WB_2, // no changes, or at least not
					    // important
...
static const struct dpu_intf_cfg sm8750_intf[] = {
	{
		.name = "intf_0", .id = INTF_0, // many new registers and fields
						// added, butthat might be only
						// for new features
...
static const struct dpu_perf_cfg sm8750_perf_data = {
	.max_bw_low = 18900000, // ok ...
	.max_bw_high = 28500000,
	.min_core_ib = 2500000,
	.min_llcc_ib = 0,
	.min_dram_ib = 800000, // ok ^^^
	.min_prefill_lines = 35,
	/* FIXME: lut tables */
	// TODO: all below are unchecked
...
static const struct dpu_mdss_version sm8750_mdss_ver = { // ok
	.core_major_ver = 12,
	.core_minor_ver = 0,
};
